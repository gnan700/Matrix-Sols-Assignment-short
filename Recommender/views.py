...
def knn(request, movie_title):
    ...
    movie_list_recommended_knn = recommenderKNN(movie_title, mat_movies_users, model_knn, 19)
...
def train_test_knn(request):
    ...
    movies_users = data_frame_of_rating_records.reset_index().pivot_table(index='movie_id', columns='user_id',
                                                                          values='rating').fillna(0)
    mat_movies_users = csr_matrix(movies_users.values)
    model_knn = KNeighborsClassifier(metric='cosine', algorithm='brute', n_neighbors=19, n_jobs=-1)
    ...
    template = 'recommender/test.html'
